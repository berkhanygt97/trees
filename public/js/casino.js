...
export class Casino {
  ...
  _lights() {
    // The ambient and fill lights are global, so the sky blends them between
  ...
  _shell() {
    ...
    // Inner walls. UVs are scaled to each panel so the pattern keeps one size
    // across the pieces the doorways cut the front wall into.
    const wallMat = lam(0xffffff, { map: wallTexture() });
    const panel = (x0, x1, y0, y1, pos, ry) => {
      const geo = new THREE.PlaneGeometry(x1 - x0, y1 - y0);
      const uv = geo.attributes.uv;
      for (let i = 0; i < uv.count; i++) {
        uv.setXY(i, (x0 + uv.getX(i) * (x1 - x0)) / w, (y0 + uv.getY(i) * (y1 - y0)) / H);
      }
    ...
    panel(0, w, 0, H, V(0, H / 2, ROOM.MIN_Z), 0);
    panel(0, d, 0, H, V(ROOM.MIN_X, H / 2, 0), Math.PI / 2);
    panel(0, d, 0, H, V(ROOM.MAX_X, H / 2, 0), -Math.PI / 2);

    // Front wall, facing inwards (rotated half a turn, so x runs backwards).
    const DOOR_H = 4.6;
    let x = ROOM.MIN_X;
    const front = [];
    for (const [a, b] of CASINO.DOORS) { front.push([x, a]); x = b; }
    front.push([x, ROOM.MAX_X]);
    for (const [a, b] of front) {
      panel(ROOM.MAX_X - b, ROOM.MAX_X - a, 0, H, V((a + b) / 2, H / 2, ROOM.MAX_Z), Math.PI);
    }
    for (const [a, b] of CASINO.DOORS) {
      panel(ROOM.MAX_X - b, ROOM.MAX_X - a, DOOR_H, H, V((a + b) / 2, (DOOR_H + H) / 2, ROOM.MAX_Z), Math.PI);
    }
  ...
  _exterior(doorH) {
    const H = ROOM.WALL_H;
    const T = CASINO.WALL;
    const e = 0.04;   // keeps outer faces off the inner panels (no z-fighting)
    const outer = lam(0x2a1834, { shininess: 20 });
    const trim = lam(0xf2c14e, { shininess: 60 });
    ...
    box(MIN_X - T, MAX_X + T, 0, H, MIN_Z - T, MIN_Z - e);
    box(MIN_X - T, MIN_X - e, 0, H, MIN_Z - T, MAX_Z + T);
    box(MAX_X + e, MAX_X + T, 0, H, MIN_Z - T, MAX_Z + T);
    let x = MIN_X - T;
    for (const [a, b] of CASINO.DOORS) {
      box(x, a, 0, H, MAX_Z + e, MAX_Z + T);
      box(a, b, doorH, H, MAX_Z + e, MAX_Z + T);
      x = b;
    }
    box(x, MAX_X + T, 0, H, MAX_Z + e, MAX_Z + T);

    // Roof with a gold parapet.
    box(MIN_X - T, MAX_X + T, H + 0.05, H + 0.7, MIN_Z - T, MAX_Z + T, lam(0x1a1020));
    box(MIN_X - T - 0.2, MAX_X + T + 0.2, H + 0.7, H + 1.1, MAX_Z + T - 0.4, MAX_Z + T + 0.2, trim);
    box(MIN_X - T - 0.2, MAX_X + T + 0.2, 0.9, 1.2, MAX_Z + T, MAX_Z + T + 0.15, trim);

    // Glowing door frames and awnings, so the entrance reads from the street.
    const glow = basic(0xffd27a);
    for (const [a, b] of CASINO.DOORS) {
      const z = MAX_Z + T + 0.1;
      box(a - 0.25, a, 0, doorH, MAX_Z + T, z, glow);
      box(b, b + 0.25, 0, doorH, MAX_Z + T, z, glow);
      box(a - 0.25, b + 0.25, doorH, doorH + 0.25, MAX_Z + T, z, glow);
      const awning = box(a - 1, b + 1, doorH + 0.6, doorH + 0.9, MAX_Z + T, MAX_Z + T + 2.6, lam(0xa81f3c));
      awning.rotation.x = -0.12;
      const mat = box(a, b, 0.005, 0.02, MAX_Z + T, MAX_Z + T + 3, lam(0x7a1024));
      mat.receiveShadow = false;
    }

    // Marquee bulbs along the front.
    const bulbs = new THREE.InstancedMesh(new THREE.SphereGeometry(0.16, 8, 6), basic(0xfff3c4), 80);
  ...
  paintBoard(rows, meId) {
    ...
    g.fillText('T H E   R I C H   L I S T', w / 2, 74);
    ...
    if (!top.length) {
      g.fillStyle = '#5c5268';
      g.fillText('nobody has any money yet...', 70, 180);
    }
    top.forEach((r, i) => {
      const y = 150 + i * 48;
      g.fillStyle = r.id === meId ? '#f2c14e' : r.online ? '#efe6d6' : '#8d8378';
      g.fillText(`${i + 1}.`, 60, y);
      g.fillText(r.name.slice(0, 16), 120, y);
      g.textAlign = 'right';
      g.fillStyle = '#6bd66b';
      g.fillText(`$${Math.round(r.netWorth).toLocaleString('en-US')}`, w - 60, y);
      g.textAlign = 'left';
    });
    texture.needsUpdate = true;
  }
